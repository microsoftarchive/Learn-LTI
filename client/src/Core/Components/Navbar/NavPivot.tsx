...
const NavPivotInner = ({ styles }: IStylesOnly<NavWrapperStyles>): JSX.Element | null => {
  ...
  const getMappedLinks = (assignment: Assignment): INavLink[] =>
    _.map(getNavLinks(assignment), link => {
      ...
      return { ...link, url: url, key: url, title: '', search: uriSearchParamsMap.get(link.name) };
    });
    ...
    return (
      <div className={classes.root}>
        <div className={classes.navArea}>
          <Pivot selectedKey={selectedNavKey} onLinkClick={handleLinkClick} styles={themedClassNames(navStyles)}>
            {_.map(getMappedLinks(assignmentStore.assignment), link => {
              let urlWithSearchParams =
                link.url + (link.search !== undefined && link.search.length > 0 ? '?' + link.search : '');
              return (
                <PivotItem
